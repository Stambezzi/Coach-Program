...
namespace MPK_Calendar
{
	...
	public class MPK_Calendar : System.Windows.Forms.UserControl
	{
        ...
        private Color dateForPaitingColor = Color.Green;
        ...
        private Color ForPaitingDarker;
        ...
        private SolidBrush brushDateForPaiting;
        private SolidBrush brushDateForPaitingDarker;
        ...
        public DateTime StartingDateClient
        {
            get
            {
                return startingDateClient;
            }
            set
            {
                startingDateClient = value;
                //intMonth = dateForPaiting.Month;
                //intYear = dateForPaiting.Year;
            }
        }
        ...
        public Color DateForPaitingColor
        {
            get
            {
                return dateForPaitingColor;
            }
            set
            {
                dateForPaitingColor = value;
            }
        }
		/// <summary>
				...
				for(int j = 0;j<6;j++)
				{
					for(int i = 0;i<7;i++)
					{
						...
						if(arrDates[i,j].Month==pintMonth)
						{
                           ...
                           /* if (dateTest.Date == this.DateForPaiting.Date)
                            {
                                bForPaiting = true;
                            }
                            else
                        ...
                        int currentWeek;
                        currentWeek = calculateCurrentWeek(arrDates[i, j]);
                        dateForPaitingColor = getColor(currentWeek);
                        brushDateForPaiting = new SolidBrush(dateForPaitingColor);
                        ForPaitingDarker = Color.FromArgb((int)(dateForPaitingColor.R * 0.8), (int)(dateForPaitingColor.G * 0.8), (int)(dateForPaitingColor.B * 0.8));
                        brushDateForPaitingDarker = new SolidBrush(ForPaitingDarker);
                        ...
                        /*else if (bForPaiting)//custom
                        {
                            g.FillRectangle(brushDateForPaiting, rects[i, j]);
                        ...
                        else if (bActive)//not selected but active
                        {
                            ...
                            {
                                g.FillRectangle(brushDateForPaitingDarker, rects[i, j]);
                            }
                            else//weekday
                            {
                                g.FillRectangle(brushDateForPaiting, rects[i, j]);
        ...
        private int calculateCurrentWeek(DateTime currentDate)
        {
            double daysPassedd;
            int daysPassed;
            int days;
            int CurrentWeek = 4;
            daysPassedd = (currentDate - StartingDateClient).TotalDays;
            daysPassed = (Int32)daysPassedd;
            days = (Int32)daysPassedd;
            daysPassed = daysPassed % (CycleLength * 7);
            if (daysPassed <= 0 && currentDate != startingDateClient && days < 0) CurrentWeek = 0;
            else if (daysPassed < 7) CurrentWeek = 1;
        ...
        private Color getColor(int currentWeek)
        {
            if (currentWeek == 0) return p_ActiveMonthColor;
            if (currentWeek == 1) return Color.Red;
            if (currentWeek == 2) return Color.Green;
            if (currentWeek == 3) return Color.Blue;
            if (currentWeek == 4) return Color.Yellow;
            if (currentWeek == 5) return Color.Pink;
            if (currentWeek == 6) return Color.Orange;
            if (currentWeek == 7) return Color.LightBlue;
